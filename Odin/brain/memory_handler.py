...
class MemoryHandler:
    ...
    def train_word2vec(self, chunks):
        """
        Train Word2Vec on all chunks in a single batch.
        """
        # Preprocess all chunks into sentences
        sentences = [simple_preprocess(chunk) for chunk in chunks]
        if not self.word2vec_model:
            # If no model exists, create a new one
            self.word2vec_model = Word2Vec(sentences, vector_size=self.vector_size, window=5, min_count=1, workers=4)
        else:
            # If a model exists, update it with the new sentences
        ...
        self.word2vec_model.save(self.model_path)
        logging.info("Word2Vec model retrained on all chunks in a single batch.")
